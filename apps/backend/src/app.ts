...
export function createApp(config: AppConfig, logger: Logger): express.Express {
  ...
  app.get(frontendRoutes, (_request, response) => { response.sendFile(resolve(publicDir, "index.html")); });
  app.use((_request, response) => { response.status(404).json({ error: "Not found" }); });
  app.use((error: unknown, _request: Request, response: Response, _next: NextFunction) => {
    logger.error("Request failed", { error: serializeError(error) });
    const publicError = toPublicError(error);
    response.status(isAppError(error) ? 400 : 500).json({ error: publicError });
  });
  return app;
}
