...
export class Ga4Service {
  ...
  private async getPageReport(startDate: string, endDate: string, limit: number): Promise<any> { validateRange(startDate, endDate); return this.runPageReport(await this.getAuthenticatedToken(), startDate, endDate, [{ name: "pagePath" }, { name: "pageTitle" }], [{ name: "sessions" }, { name: "activeUsers" }, { name: "screenPageViews" }], undefined, limit, "screenPageViews"); }
