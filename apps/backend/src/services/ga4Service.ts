...
const WEBSITE_EXCLUDED_SESSION_SOURCES = ["trafficheap.cc"];
...
export class Ga4Service {
  ...
  async test(startDate: string, endDate: string): Promise<unknown> {
    ...
    return { configured: true, propertyId: this.config.ga4.propertyId, dateRange: { startDate, endDate }, website, websiteExclusions: WEBSITE_EXCLUDED_SESSION_SOURCES, trafficQuality, suspiciousReferral, topPages: this.mapRows(report.rows ?? []).slice(0, 25), rowCount: report.rowCount ?? 0, writesPerformed: false };
  }
  ...
  private async getWebsiteSummary(startDate: string, endDate: string): Promise<Ga4WebsiteMetrics> {
    validateRange(startDate, endDate); const accessToken = await this.getAuthenticatedToken();
    const payload = await this.runReport(accessToken, startDate, endDate, [], [{ name: "sessions" }, { name: "activeUsers" }, { name: "screenPageViews" }, { name: "engagedSessions" }, { name: "engagementRate" }, { name: "bounceRate" }], websiteTrafficFilter()); const values = payload.rows?.[0]?.metricValues ?? [];
...
function websiteTrafficFilter(): unknown {
  return { notExpression: { filter: { fieldName: "sessionSource", inListFilter: { values: WEBSITE_EXCLUDED_SESSION_SOURCES, caseSensitive: false } } } };
}
