...
export class Ga4Service {
  ...
  async test(startDate: string, endDate: string): Promise<unknown> {
    const [report, website, trafficQuality] = await Promise.all([
      this.getPageReport(startDate, endDate, 100),
      this.getWebsiteSummary(startDate, endDate),
      this.trafficQuality(startDate, endDate)
    ]);
    return { configured: true, propertyId: this.config.ga4.propertyId, dateRange: { startDate, endDate }, website, trafficQuality, topPages: this.mapRows(report.rows ?? []).slice(0, 25), rowCount: report.rowCount ?? 0, writesPerformed: false };
  }
  ...
  async trafficQuality(startDate: string, endDate: string): Promise<unknown> {
    ...
    const response = await fetch(`https://analyticsdata.googleapis.com/v1beta/properties/${encodeURIComponent(this.config.ga4.propertyId)}:runReport`, {
      ...
      body: JSON.stringify({ dateRanges: [{ startDate, endDate }], dimensions: [{ name: "sessionSourceMedium" }], metrics: [{ name: "sessions" }, { name: "engagedSessions" }, { name: "engagementRate" }, { name: "bounceRate" }, { name: "screenPageViews" }], orderBys: [{ metric: { metricName: "sessions" }, desc: true }], limit: 50 })
    });
    const payload = await response.json() as any; if (!response.ok) throw new Error(`GA4 traffic quality request failed (${response.status}): ${payload.error?.message ?? response.statusText}`);
  ...
  private async getWebsiteSummary(startDate: string, endDate: string): Promise<Ga4WebsiteMetrics> {
    validateRange(startDate, endDate); const accessToken = await this.getAuthenticatedToken();
    const response = await fetch(`https://analyticsdata.googleapis.com/v1beta/properties/${encodeURIComponent(this.config.ga4.propertyId)}:runReport`, { method: "POST", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify({ dateRanges: [{ startDate, endDate }], metrics: [{ name: "sessions" }, { name: "activeUsers" }, { name: "screenPageViews" }, { name: "engagedSessions" }, { name: "engagementRate" }, { name: "bounceRate" }] }) });
    const payload = await response.json() as any; if (!response.ok) throw new Error(`GA4 summary request failed (${response.status}): ${payload.error?.message ?? response.statusText}`); const values = payload.rows?.[0]?.metricValues ?? [];
  ...
  private async getPageReport(startDate: string, endDate: string, limit: number): Promise<any> { validateRange(startDate, endDate); return this.runPageReport(await this.getAuthenticatedToken(), startDate, endDate, limit); }
  ...
  private async runPageReport(accessToken: string, startDate: string, endDate: string, limit: number): Promise<any> { const response = await fetch(`https://analyticsdata.googleapis.com/v1beta/properties/${encodeURIComponent(this.config.ga4.propertyId)}:runReport`, { method: "POST", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify({ dateRanges: [{ startDate, endDate }], dimensions: [{ name: "pagePath" }, { name: "pageTitle" }], metrics: [{ name: "sessions" }, { name: "activeUsers" }, { name: "screenPageViews" }], orderBys: [{ metric: { metricName: "screenPageViews" }, desc: true }], limit }) }); const payload = await response.json() as any; if (!response.ok) throw new Error(`GA4 Data API request failed (${response.status}): ${payload.error?.message ?? response.statusText}`); return payload; }
}
