...
export class CastosImportConnector extends BaseConnector {
  readonly metadata = {
    ...
    sourceName: "Castos",
    description: "Imports one or more monthly Castos listens analytics exports in a single batch",
  ...
  override async authenticate(context: ConnectorRunContext) {
    return context.csv?.trim()
      ? { ok: true, status: "Connected" as const, message: "Castos listens export batch ready." }
      : { ok: false, status: "Needs Setup" as const, message: "Choose one or more Castos Listens CSVs before importing." };
  }

  override async fetchMetrics(context: ConnectorRunContext): Promise<RawConnectorMetric[]> {
    const bundle = parseBundle(context.csv ?? "");
    if (!bundle.files.length) throw new AppError("VALIDATION_FAILED", "No Castos CSV files were provided.");
    ...
    for (const file of bundle.files) {
      if (detectType(file.csv) !== "listens") {
        throw new AppError("VALIDATION_FAILED", `${file.name} is not a Castos Listens export. Expected columns: Published Date, Podcast, Episode, Listens.`);
      }

      const range = parseFilenameRange(file.name);
      if (!range) {
        throw new AppError("VALIDATION_FAILED", `Could not determine the Castos reporting period from ${file.name}. Download the report directly from Castos and do not rename it before importing.`);
      }
      if (!isFullCalendarMonth(range)) {
        throw new AppError("VALIDATION_FAILED", `${file.name} covers ${range.start} to ${range.end}. Castos does not include individual listen dates, so a multi-month file cannot be split accurately. Export one CSV per calendar month, then select all of those monthly files together in one upload.`);
      }

      const periodKey = `${range.start}|${range.end}`;
      if (seenPeriods.has(periodKey)) {
        throw new AppError("VALIDATION_FAILED", `More than one Castos file was supplied for ${range.start} to ${range.end}. Remove the duplicate month and try again.`);
      }
...
function parseListens(csv: string) {
  ...
  if (Object.values(indexes).some((index) => index < 0)) throw new AppError("VALIDATION_FAILED", "The Castos Listens export does not have the expected columns.");
  const parsed = rows.slice(1).map((row, index) => ({ publishedDate: requireDate(row[indexes.publishedDate], index + 2), podcast: String(row[indexes.podcast] ?? "").trim(), episode: String(row[indexes.episode] ?? "").trim(), listens: requireNumber(row[indexes.listens], index + 2) }));
...
function parseFilenameRange(name: string): DateRange | undefined {
  ...
  const start = dateParts(Number(match[1]), Number(match[2]), Number(match[3])); const end = dateParts(Number(match[4]), Number(match[5]), Number(match[6]));
  return start && end ? { start, end } : undefined;
}
function isFullCalendarMonth(range: DateRange): boolean {
  const startMatch = /^(\d{4})-(\d{2})-(\d{2})$/.exec(range.start); const endMatch = /^(\d{4})-(\d{2})-(\d{2})$/.exec(range.end);
  if (!startMatch || !endMatch || startMatch[1] !== endMatch[1] || startMatch[2] !== endMatch[2] || startMatch[3] !== "01") return false;
  const year = Number(startMatch[1]); const month = Number(startMatch[2]); const lastDay = new Date(Date.UTC(year, month, 0)).getUTCDate();
...
function requireNumber(value: unknown, line: number) { const text=String(value??"").trim().replace(/,/g,"");const parsed=Number(text);if(!text||!Number.isFinite(parsed)||parsed<0)throw new AppError("VALIDATION_FAILED",`Castos CSV row ${line} contains an invalid listens value.`);return parsed; }
function requireDate(value: unknown, line: number) { const date=normalizeDate(String(value??""));if(!date)throw new AppError("VALIDATION_FAILED",`Castos CSV row ${line} contains an invalid published date.`);return date; }
