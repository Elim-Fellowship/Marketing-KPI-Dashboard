...
export class SpotifyImportConnector extends BaseConnector {
  ...
  override async fetchMetrics(context: ConnectorRunContext): Promise<RawConnectorMetric[]> {
    return parseSpotifyCsv(context.csv ?? "").map((row) => ({
      sourceRecordId: key(row.episodeName, row.publishDate),
      metricName: "streams",
      value: row.totalStreams,
      unit: "streams",
      date: row.publishDate,
      targetTableKey: "spotifyEpisodeMetrics",
      platform: "Spotify",
      channel: "Spotify",
      contentTitle: row.episodeName,
      contentType: "Podcast Episode",
      activityVolume: 1,
      dimensions: { episodeName: row.episodeName, likes: row.likes ?? 0, downloads: row.downloads ?? 0 }
    }));
  }
    ...
    return {
      ...
      records: metrics.map((metric) => {
        ...
        const fields: AirtableFields = {
          ...
          "Total Streams": metric.value,
          Source: "spotify",
          "Import Date": new Date().toISOString().slice(0, 10)
        };
        const likes = positiveNumber(metric.dimensions?.likes);
        const downloads = positiveNumber(metric.dimensions?.downloads);
        if (likes !== undefined) fields.Likes = likes;
        if (downloads !== undefined) fields.Downloads = downloads;
        return {
          tableKey: "spotifyEpisodeMetrics" as const,
          uniqueKey: { fieldName: "Episode Name", value: key(episodeName, metric.date) },
          fields
        };
      })
    };
  ...
  override async writeToAirtable(payload: ConnectorAirtablePayload, context: ConnectorRunContext): Promise<ConnectorWriteResult> {
    ...
    for (const record of existing) {
      const name = String(record.fields["Episode Name"] ?? "").trim();
      const date = normalizeDate(String(record.fields["Publish Date"] ?? ""));
      if (name && date && !byKey.has(key(name, date))) byKey.set(key(name, date), record);
    }
    ...
    for (const record of payload.records) {
      const name = String(record.fields["Episode Name"] ?? "").trim();
      const date = normalizeDate(String(record.fields["Publish Date"] ?? ""));
      const recordKey = key(name, date ?? "");
      const current = byKey.get(recordKey);
...
function parseSpotifyCsv(csv: string): SpotifyRow[] {
  ...
  const episode = find(headers, ["episode name", "episode", "episode title", "title", "name"]);
  const date = find(headers, ["publish date", "published", "published date", "release date", "date"]);
  const streams = find(headers, ["total streams", "streams", "plays", "spotify plays", "starts"]);
  const recognized = episode >= 0 || date >= 0 || streams >= 0;

  if (recognized && (episode < 0 || date < 0 || streams < 0)) {
    throw new AppError("VALIDATION_FAILED", "Spotify CSV needs episode name, publish date, and streams/plays columns.", { headers: rows[0] });
  }

  const indexes = recognized
    ? { episode, date, streams, likes: find(headers, ["likes", "total likes"]), downloads: find(headers, ["downloads", "total downloads"]) }
    : { episode: 0, streams: 1, date: 2, likes: -1, downloads: -1 };

  const data = recognized ? rows.slice(1) : rows;
  const parsed: SpotifyRow[] = [];
  const errors: string[] = [];

  data.forEach((row, index) => {
    const line = index + (recognized ? 2 : 1);
    const episodeName = String(row[indexes.episode] ?? "").trim();
    const publishDate = normalizeDate(String(row[indexes.date] ?? ""));
    const totalStreams = parseNumber(row[indexes.streams]);
    if (!episodeName && !publishDate && totalStreams === undefined) return;
    if (!episodeName) errors.push(`Row ${line}: missing episode name`);
    if (!publishDate) errors.push(`Row ${line}: invalid publish date`);
    if (totalStreams === undefined) errors.push(`Row ${line}: invalid streams/plays value`);
    if (episodeName && publishDate && totalStreams !== undefined) {
      parsed.push({
        episodeName,
        publishDate,
        totalStreams,
        likes: indexes.likes >= 0 ? parseNumber(row[indexes.likes]) : undefined,
        downloads: indexes.downloads >= 0 ? parseNumber(row[indexes.downloads]) : undefined
      });
    }
  });
  ...
  const unique = new Map<string, SpotifyRow>();
  for (const row of parsed) unique.set(key(row.episodeName, row.publishDate), row);
