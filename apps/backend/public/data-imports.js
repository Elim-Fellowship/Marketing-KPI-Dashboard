...
function setFile(file) {
  ...
  if (!file.name.toLowerCase().endsWith(".csv")) {
    showResult("error", "Please select a CSV file exported from Spotify for Creators.");
    currentFile = undefined;
    selectedFile.textContent = "No file selected";
    updateButtonState();
    return;
...
function updateButtonState() {
  importButton.disabled = !currentFile || !importKey.value.trim();
}
...
async function importSpotifyCsv() {
  ...
  try {
    ...
    if (!response.ok) {
      const message = body?.error?.message ?? body?.error?.code ?? `Import failed (${response.status})`;
      throw new Error(message);
    }

    const result = body?.result?.results?.[0];
    if (!result || result.status !== "Success") {
      throw new Error(result?.errorMessage ?? "Spotify import did not complete successfully.");
    }

    const write = result.writeResult ?? {};
    showResult(
      "success",
      `<strong>Spotify analytics imported successfully.</strong><div class="result-metrics"><span>${number(write.created)} new</span><span>${number(write.updated)} updated</span><span>${number(write.skipped)} unchanged</span><span>${number(result.recordsPrepared)} processed</span></div><p>Channel Breakdown will use the imported episode records the next time it loads.</p>`
    );
...
async function loadHistory() {
  try {
    ...
    if (!history.length) {
      historyRoot.innerHTML = '<div class="empty-state">No Spotify imports have run in this server session yet.</div>';
      return;
    }
    ...
    historyRoot.innerHTML = `
      <div class="history-table-wrap">
        <table class="history-table">
          <thead><tr><th>Time</th><th>Status</th><th>Processed</th><th>New</th><th>Updated</th><th>Unchanged</th></tr></thead>
          <tbody>${history.map(renderHistoryRow).join("")}</tbody>
        </table>
      </div>`;
  } catch (error) {
